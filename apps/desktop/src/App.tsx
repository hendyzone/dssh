...
export default function App() {
  ...
  const deleteServer = (id: string) => {
    ...
    saveServers(next);
    // 顺带关掉该服务器的会话
    sessions.filter((s) => s.server.id === id).forEach((s) => closeSession(s.id));
  };
    ...
    <div className="app">
      ...
      <main className="main-area">
        ...
        {sessions.length === 0 ? (
          ...
          sessions.map((s) => (
            <div key={s.id} className="terminal-wrapper" hidden={s.id !== activeId}>
              <TerminalView session={s} active={s.id === activeId} />
            </div>
          ))
        )}
      </main>
      {showForm && <ServerForm onSubmit={addServer} onCancel={() => setShowForm(false)} />}
    </div>
  );
}
