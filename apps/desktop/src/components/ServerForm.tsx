...
export default function ServerForm({ onSubmit, onCancel }: Props) {
  ...
  const valid = host.trim() !== '' && username.trim() !== '' && Number(port) > 0;
  ...
  const submit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      passphrase: authMethod === 'publicKey' && passphrase ? passphrase : undefined,
    });
  };
  ...
  return (
    <div className="modal-backdrop" onClick={onCancel}>
      <form className="modal" onClick={(e) => e.stopPropagation()} onSubmit={submit}>
        ...
        <label>
          地址 *
          <input value={host} onChange={(e) => setHost(e.target.value)} placeholder="192.168.1.1 或 example.com" autoFocus />
        </label>
        ...
        <div className="form-actions">
          ...
          <button type="submit" disabled={!valid}>保存并连接</button>
        </div>
      </form>
    </div>
  );
