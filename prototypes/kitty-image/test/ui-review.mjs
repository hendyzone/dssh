...
await page.addInitScript(() => {
  ...
  window.__TAURI_INTERNALS__ = {
    ...
    invoke: async (cmd, args) => {
      if (cmd === 'servers_list') {
        return [
          { id: 's1', name: '生产 Web 服务器', host: '10.0.1.10', port: 22, username: 'deploy', group: '生产环境', authMethod: 'publicKey', keyPath: '~/.ssh/id_ed25519', hasPassphrase: true },
          { id: 's2', name: '生产数据库', host: '10.0.1.20', port: 22, username: 'root', group: '生产环境', authMethod: 'password', hasPassword: true },
          { id: 's3', name: '测试机', host: '192.168.31.100', port: 2222, username: 'deng', group: '测试', authMethod: 'password', hasPassword: true },
          { id: 's4', name: '家里的 NAS', host: 'nas.home.lan', port: 22, username: 'admin', authMethod: 'password', hasPassword: true },
        ];
      }
...
await page.click('.modal-backdrop', { position: { x: 1200, y: 100 } }).catch(() => {});
