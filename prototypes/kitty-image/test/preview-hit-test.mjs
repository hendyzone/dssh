...
await page.keyboard.type(
  "bash /srv/workspace/dssh/prototypes/kitty-image/server/test-image.sh",
);
...
const result = await page.evaluate(() => {
  ...
  const hit =
    col >= p.viewportCol &&
    col < p.viewportCol + p.gridCols &&
    row >= p.viewportRow &&
    row < p.viewportRow + p.gridRows;
...
console.log(
  `2. placement 命中 ✓ grid(${result.placement.col},${result.placement.row} ${result.placement.cols}x${result.placement.rows}格) 自检=${result.hitTestSelfCheck}`,
);
console.log(
  `3. 像素提取 ✓ ${result.pixels.width}x${result.pixels.height} format=${result.pixels.format}`,
);
